...
class Ion_auth
{
	...
	public function __construct()
	{
        ...
        $this->from_email = $this->ion_auth_model->get_sender_email(); 
	}
      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			...
			if ($user)
			{
				...
				else
				{
					...
					/*$this->email->clear();
					$this->email->from($this->from_email, $this->config->item('site_title', 'ion_auth'));
                    ...
                    */
                    ...
                    $email_title = $this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject');
                    $to = array($user->email);
                    
                    $this->notifications->send_emails_notifications($email_title, $message , $to);
