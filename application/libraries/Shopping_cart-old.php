...
class Shopping_cart
{
    ...
    public function update($product_id, $updated_data, $stock_product=0)
    {
        $cart_id   = $this->get_cart_id();
        $cart_data = $this->shopping_cart_data();
        $product_cart_data  = $this->get_product_details($product_id);
    ...
    {
        $cart_id      = $this->get_cart_id();
        $product_data = $this->CI->shopping_cart_model->get_product_data($cart_id, $product_id);
    ...
    public function get_cart_product_data($product_id)
    {
        $cart_id      = $this->CI->shopping_cart_model->get_cart_id($this->user_id, $this->session_id, $this->ip_address);
        $product_data = $this->CI->shopping_cart_model->get_product_data($product_id, $cart_id);
    ...
    public function coupon_discount($coupon_code)
    {
        $cart_id       = $this->get_cart_id();
        $cart_contents = $this->contents();
        $cart_data     = $this->shopping_cart_data();
    ...
    private function _check_coupon($cart_content, $coupon_data, $cart_id)
    {
        ...
        if($coupon_data->product_or_category == 'total')
        {
            ...
            $price = $cart_data->total_price - $cart_data->discount;
    ...
    public function check_cart_products_quantities($new_qties= array())
    {
        ...
        if(count($new_qties) == 0)
        {
            ...
            foreach ($contents as $content)
            {
                $new_qties[$content->product_id] = $content->qty;
            }
        }
        ...
        foreach($new_qties as $product_id=>$qty)
        {
            if($product_id == 0)
            {
                $status = '1';
            ...
            $status_array[$product_id] = $status;
        }
            ...
            if($shipping_company_id != 0)
            {
                ...
                if($shipping_company_data->type == 1)
                {
                    ...
                    else
                    {
                        ...
                        $cost = $shipping_company_data->cost;
                        $total_shipping_cost = $cost * $cart_data->total_weight;
                        //check maintenance products in cart

                        $maintenance_products = $this->CI->shopping_cart_model->check_maintenance_products_exist($this->cart_id, $this->settings->maintenance_cat_id);

                        if($maintenance_products)
                        {
