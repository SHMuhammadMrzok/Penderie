...
class Amazon_s3_uploads
{
    ...
    public function upload_to_o3($image_name, $upload_folder='', $map_code='')
    {

      /// AWS API keys
      $aws_access_key_id = $this->CI->config->item('amazon_s3_access_key');//'AKIAJSBXCQ7BCMQVDJNA';
      $aws_secret_access_key = $this->CI->config->item('amazon_s3_secret_key');//'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';

      // Bucket
      $bucket_name = $this->CI->config->item('amazon_s3_my_bucket');//'sbmcart';
      $subfolder   = $this->CI->config->item('amazon_s3_subfolder');//'qahwastore/';
      ...
      $aws_region = $this->CI->config->item('amazon_s3_region');//'eu-west-2';
      $host_name = $bucket_name . '.s3.amazonaws.com';

      if($map_code == '')
      {
        // Server path where content is present. This is just an example
          $content_path = realpath(APPPATH. '../assets/uploads/'.$upload_folder.'/'.$image_name);
          //'/home/sbmcart/qahwastore.sbmcart.com/assets/uploads/products/f0cca-products-arabic-coffee-350x350.jpg';//'img/color.png';
      ...
      $content_title = $subfolder.$image_name;
