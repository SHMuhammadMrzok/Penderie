...
class Optional_fields extends CI_Controller
{
    ...
    public function ajax_list()
    {
     ...
     public function add()
     {
        ...
        if(strtoupper($_SERVER['REQUEST_METHOD']) == 'POST')
        {
            ...
            foreach($languages as $lang_id)
            {
                ...
                if($has_options == 1)
                {
                    $this->form_validation->set_rules('new_option_value', lang('option_value'), 'callback_check_added_options');
                }
            }
        ...
        else
        {
            ...
            $option_data   = array(
                                    ...
                                    'is_moquette'   => (isset( $_POST['is_moquette']))? $_POST['is_moquette']:0,
            ...
            else
            {
                ...
                if($this->optional_fields_model->update_option_field($id, $option_data))
                {
                    ...
                    if(isset($_POST['has_options']) && $_POST['has_options'] == 1)
                    {
                        ...
                        // update exist option options
                        $exist_option_values   = $this->input->post('option_value');
                        $exist_option_priority = $this->input->post('sort');
                        
                        foreach($exist_option_priority as $op_id=>$value)
                        {
                            $option_data = array(
                                                    'priority' => $value
                                                 );
                            
                            $this->optional_fields_model->update_optional_field_option($op_id, $option_data);
                            
                            foreach($languages as $lang_id)
                            {
                                $exist_option_translation_data = array(
                                                                   'field_value' => $exist_option_values[$lang_id][$op_id]
                                                                      );
                                
                                $this->optional_fields_model->update_optional_field_option_translation($op_id, $lang_id, $exist_option_translation_data);
                            }
                        }
     ...
     public function check_added_options($options)
     {
        $languages       = $this->input->post('lang_id');
        $option_priority = $this->input->post('new_sort');
        $options_values  = $this->input->post('new_option_value');
        
        if(!isset($_POST['new_option_value']) )
        {
            if(! isset($_POST['option_value']) && count($_POST['option_value']) == 0)
            {
                $this->form_validation->set_message('check_added_options', lang('options_required'));
                return FALSE;
            }
        }
        else if(!isset($_POST['new_sort']) || count(array_filter($_POST['new_sort'])) == 0)
        {
     ...
     public function delete_option_option()
     {
        $optional_field_option_id = intval($this->input->post('optional_field_option_id', true));
        
        // check if option i used before delete
        $op_products_count = $this->optional_fields_model->check_op_products($optional_field_option_id);
        
        if($op_products_count > 0)
        {
            echo "0";
        }
        else
        {
            // delete option
            $this->optional_fields_model->delete_options_option($optional_field_option_id);
            echo "1";
        }
     }
