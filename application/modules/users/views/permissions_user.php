...
<form id="permissions_user_form" method="post" action="<?php echo base_url()?>users/permissions_user/save_permission" class="form-horizontal form-bordered">
                            ...
                            <?php  
                                    foreach($controllers as $controller){ 
                                        if($controller->module_id== $module_id){
                                             $controller_check="";
                                            if(in_array($controller->id,$user_old_controllers))
                                            {
                           ...
                           <li role="treeitem" id="c_<?php echo $controller->id; ?>" aria-selected="false" aria-expanded="true" data-jstree='{ "icon" : "<?php echo ($controller->icon_class=='')? 'fa  fa-file-o' : $controller->icon_class; ?> icon-state-danger ","opened" : true }'>
                                ...
                                <?php  //print_r($permissions) ;die();
                                    ...
                                    foreach($permissions["{$controller->id}"] as $perm)
                                        {
                                            $permission_check="";
                                            if(isset($user_old_permissions["{$controller->id}"]))
                                            {
                                                if(in_array($perm->id,$user_old_permissions["{$controller->id}"]))
                                                {
