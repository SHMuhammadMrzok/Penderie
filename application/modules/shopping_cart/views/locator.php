...
?>
    ...
    <div id="searchbuttons" style="text-align: center;" class="row form-group margin-bottom-20px">
        <div class="col-sm-8"> 
            <input type="text" id="autocomplete" name="autocomplete" placeholder="<?php echo lang('type_in_address'); ?>"  class="form-control" />
        </div>
        <div class="col-sm-2"> 
            <input type="button" value="<?php echo lang('locate_location'); ?>" onclick="branchlocate();"  class="btn btn-default create-button" />
        </div>
    </div>
...
// get other location
      function branchlocate() {
        ...
        geodecoder.geocode({'address': myaddress}, function(results, status) {
          ...
          {
            alert('<?php echo lang('no_near_branches');?>');
            $('#submit_order').hide();
          }
          else
          {
...
function displaylistedmarkers(branches){
        var infowindow = new google.maps.InfoWindow();
        var placesList = document.getElementById('places');
        for (i = 0; i < branches.length; i++) {  
            branchdetails[i] = '<b>' + branches[i]["name"] + '</b><br>Address : ' + branches[i]["address"] + '</b><br>phone : ' + branches[i]["phone"] + '<br>distance : ' + branches[i]["distance"] + 'km.';
            createMarker(branches[i]["name"], branches[i]["lat"], branches[i]["lng"], branchdetails[i], 'no');          
            placesList.innerHTML += '<li style="cursor: pointer; cursor: hand;" onclick="selectmarker(' + i + ')";><label style="cursor: pointer; cursor: hand;"><input type="radio" name="branch_id" value="' + branches[i]["id"] + '"> ' + branchdetails[i] + '</label></li>';
        }
