...
class Create_order extends CI_Controller
{
                ...
                if($user_id != 0)
                {
                    ...
                    if($user_customer_group_data->max_orders_per_day > $user_orders_count || $user_customer_group_data->max_orders_per_day == 0)
                    {
                        ...
                        else
                        {
                            ...
                            if($payment_method_id != 10)  // if payment method is not cash on delivery then create payment form before creating the order
                            {
                              //update cart data
                              $cart_updated_data = array(
                                'agent'             => $agent,
                                'payment_option_id' => $payment_method_id                     ,
                                'gift_msg'          => $gift_msg,
                                'user_notes'        => $notes,
                                'lang_id'           => $lang_id
                              );
                              $this->shopping_cart-> update_cart($cart_data->id, $cart_updated_data);
                              //create payment page before creating order
                              $products_names = '';
                              $cart_products = $this->shopping_cart_model->get_cart_products_translation($cart_data->id, $lang_id);
                              foreach($cart_products as $item)
                              {
                                $products_names .= $item->product_name;
                              }
                              $url = $this->_generate_payment_form($payment_method_id, $cart_data->id, $cart_data->final_total_price_with_tax, $cart_data->currency_symbol, $products_names, $lang_id, $user_id, 1);
    ...
    private function _generate_payment_form($payment_method_id, $order_id, $final_total, $currency_symbol, $products_names, $lang_id, $user_id)
    {
        ...
        elseif($payment_method_id == 16)  //mastercard
        {
            $url = base_url().'orders/Payment_gateways/process_mastercard/'.$order_id.'/';
        }
        else
        {
    ...
    public function payfort_form($order_id, $lang_id, $user_id)
    {
     ...
     public function encrypt_and_update_users_data($user_id, $field, $data)
     {
         ...
         return $this->user_model->update_user_balance($user_id, $user_points_data);
