...
class Create_order extends CI_Controller
{
    ...
    public function index()
    {
        ...
        /*foreach($cart_contents as $content)
        {
            ...
            {
                $product_details    = $this->products_model->get_product_row_details($content->product_id, $lang_id, $country_id);
                ...
                {
                    $country_check_array[] = 0;
                }
                else
                {
                    $country_check_array[] = 1;
                }
                ...
                else
                {
                    ...
                    $stock_count = $this->products_model->count_product_available_quantity($content->product_id, $country_id);
                    $min_stock   = $this->config->item('min_product_stock');
        ...
        */
        ...
        /*if(in_array(0, $country_check_array))
        {
            $fail_msg = $this->general_model->get_lang_var_translation('product_not_exist_in_country', $lang_id);
            $output   = array(
                               'response' => 0,
                               'message'  => $fail_msg
                             );
        ...
        else if(in_array(0, $stock_check_array))
        {
            $fail_msg = $this->general_model->get_lang_var_translation('no_stock', $lang_id);
            $output   = array(
                               'response' => 0,
                               'message'  => $fail_msg
                             );
                    ...
                    if($user_customer_group_data->max_orders_per_day > $user_orders_count || $user_customer_group_data->max_orders_per_day == 0)
                    {
                        ...
                        elseif($payment_method_id == 2) //reward points
                        {
                            ...
                            if($this->ion_auth->login($email, $password))
                            {
                                ...
                                $user_reward_points_value = $this->api_lib->get_user_reward_points_value($userId, $country_id);
                        ...
                        if($insert_order)
                        {
                            ...
                            /*if($cart_data->shipping_company_id != 0)
                            {
                                $cart_data = $this->shopping_cart->check_cart_shipping_cost();
                            }*/
                            ...
                            foreach($cart_stores as $store)
                            {
                                ...
                                $cart_products_result = $this->calculate_order_products_prices($cart_stores_products, $order_tax, $order_wrapping_cost, $shipping_cost, $cart_data->vat_percent);
                            ...
                            if($this->status == 1)
                            {
                                ...
                                {
                                    $url = $this->_generate_payment_form($payment_method_id, $order_id, $final_total, $cart_data->currency_symbol, $products_names, $lang_id, $userId);
                                }
    ...
    public function payfort_form($order_id, $lang_id, $user_id)
    {
     ...
     public function calculate_order_products_prices($cart_products_array, $tax, $wrapping_cost, $shipping_cost, $vat_percent)
     {
        $products_discount  = 0;
        $products_price     = 0;
        $coupon_discount    = 0;
        ...
        $final_price_with_tax = $final_price + $tax + $wrapping_cost + $shipping_cost - $products_discount;
