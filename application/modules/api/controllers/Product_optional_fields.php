...
class Product_optional_fields extends CI_Controller
{
    ...
    /*public function save_user_optional_fields_old()
    {
        $output     = array();
...
*/
    ...
    public function save_user_optional_fields()
    {
        ...
        else
        {
            ...
            // Set product ordered quantity according to quantity type received ,
            // add one for quantity type 1 , add requested quantity for quantity type 2
            if($quantity_type == 1)
            {
                $product_qty    = 1;
            }
            ...
            // Flag to Allaw insertion of product with selected optionl ,
            // if there is available quantity of it's serials
            $product_options_available_quantity_flag = true;
            
            if (count($product_optional_fields) > 0 && count($user_fields) > 0 )
            {
                // Splite posted options to Optional array and selected options array
                $selected_optionals_array   = array_values(array_column($user_fields, 'optionId')); // array of the product optionals ids
                $selected_options_array     = array_values(array_column($user_fields,'optionLabel')); // array of the selected options values
                ...
                if($selected_options_available_qty < 1 || $product_qty > $selected_options_stock_count)
                {
                    ...
                    $product_options_available_quantity_flag = false ;
                }
            }
            /* END Checking the exisiting quantity of selected options */

            if ($product_options_available_quantity_flag){
                // Insert New CART PRODUCT row and it's options if there is no same product in cart , else update the exist row with new quant it
                ...
                // check that no same cart object and it is false
                if( !is_array($same_cart_product_exist) && !$same_cart_product_exist) 
                {
                    ...
                    foreach($user_fields as $field)
                    {
                        $answer    = strip_tags($field['optionLabel']);
                        $option_id = strip_tags($field['optionId']);
                        $qty       = isset($field['qty']) ? intval($field['qty']) : 1;
                    ...
                    */
                    ...
                    $data    = array(
                                    ...
                                    'price'         => $product_price_data[0]   ,
                                    'final_price'   => $product_price_data[0],//$product_final_price     ,
                                    'discount'      => $product_price_data[5]   ,
                                    'image'         => $product_details->image  ,
                                    'reward_points' => $product_details->reward_points    ,
                    ...
                    if($this->shopping_cart->update($product_details->id, $data, $non_stock_product, $cartRowId))
                    {
                        ...
                        $msg     = $this->general_model->get_lang_var_translation('product_qty_updated', $lang_id);
                        $success = 1;
                    }
                    else {
                        // quantity not updated
                        $msg     = $this->general_model->get_lang_var_translation('no_stock', $lang_id);
                        $success = 0;
                    }
