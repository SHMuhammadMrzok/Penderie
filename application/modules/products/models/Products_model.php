...
class Products_model extends CI_Model
{
   ...
   /***************Delete *************/
    ...
    public function delete_product_data($product_ids_array)
    {
        ...
        {
            echo lang('error_existing_serials');
        }
        elseif($product_purchase_order_count > 0)
        {
            echo lang('error_product_in_purchase_order');
        }
        else
        {
            $this->db->where_in('product_id',$product_ids_array);
   ...
   public function get_country_other_products($order_products_ids, $country_id, $lang_id, $store_id)
   {
    ...
    public function get_product_row_details($product_id, $lang_id, $country_id=0, $excluded_cats=array(), $no_product_lang_check=0)
    {
        $this->db->select('products.*, products_translation.*, products_countries.*,
                           stores_translation.name as store_name, brands_translation.name as brand_name,
        ...
        if($no_product_lang_check == 0)
        {
          //when lang id is set for all data
          $this->db->where('products_translation.lang_id', $lang_id);
        }
        ...
        {
            $this->db->where_not_in('products.cat_id', $excluded_cats);
        }
        //$this->db->where('products_countries.active', 1);
        //$this->db->where('products_countries.country_id', $country_id);
    ...
    public function get_products($lang_id, $limit=0, $offset=0, $country_id = 0, $cat_id=0, $store_id=0, $products_ids=array(), $excluded_cats=array(), $min_price=0)
    {
        $this->db->select('products.*, products_translation.*,categories_translation.name, products.id as id, categories_translation.name as cat_name, stores_translation.name as store_name');
        ...
        if(count($excluded_cats) != 0)
        {
            $this->db->where_not_in('categories.id', $excluded_cats);
        }

        if($min_price != 0)
        {
            $this->db->where('products_countries.price >=', $min_price);
        }
    ...
    public function get_all_products_with_limit($lang_id, $limit, $offset, $order_by=0, $order_state='desc', $start_unix_time=0, $end_unix_time=0)
    {
        $select_syn = 'products.*, products_translation.*,categories_translation.name, products.id as id, categories_translation.name as cat_name';

        if($start_unix_time !=0 && $end_unix_time !=0)
        {
            $select_syn .= " ,count(products_search.product_id) as view";
        }

        $this->db->select($select_syn);
        ...
        if($start_unix_time !=0 && $end_unix_time !=0)
        {
            $this->db->join('products_search', 'products.id = products_search.product_id');

            if($start_unix_time != 0)
            {
                $this->db->where('products_search.unix_time >= ', $start_unix_time);
            }

            if($end_unix_time != 0)
            {
                $this->db->where('products_search.unix_time < ', $end_unix_time);
            }
        ...
        if($order_by != 0)
        {
            if($order_by == 1)//most watched
            {
                $this->db->order_by('products.view', $order_state);
            }
            else
            {
    ...
    public function get_all_products_with_limit_count($lang_id, $start_unix_time=0, $end_unix_time=0)
    {
        //$this->db->select('products.*, products_translation.*,categories_translation.name, products.id as id, categories_translation.name as cat_name');
    ...
    public function get_product_price($product_id, $country_id, $all_row=0)
    {
    ...
    {
        $this->db->select('products.*, products_translation.title,
        categories.route as cat_route, categories_translation.name as cat_name,
        products_translation.title as product_name');
    ...
    public function get_count_all_products_discounts($lang_id,$search_word='', $stores_ids=array())
    {
        $this->db->join('products_translation', 'products_discounts.product_id = products_translation.product_id');
        $this->db->join('products', 'products_discounts.product_id = products.id');
    ...
    public function get_count_all_products($lang_id, $stores_ids=array(), $search_word='', $category_id=0, $store_id=0, $excluded_cats=array(), $min_price=0)
    {
        $this->db->join('products_translation', 'products.id = products_translation.product_id');
        $this->db->join('products_countries', 'products.id = products_countries.product_id', 'left');// AND products_countries.active = 1');
        $this->db->join('categories_translation', 'products.cat_id = categories_translation.category_id
                        AND categories_translation.lang_id ='.$lang_id, 'left');
        $this->db->join('categories', 'products.cat_id = categories.id', 'left');//AND categories.active = 1');
  ...
  public function count_product_available_quantity($product_id, $country_id, $optional_fields=array(), $selected_ops=array() )
  {
    $this->db->group_start();
        $this->db->where('product_id', $product_id);
        $this->db->where('serial_status', 0);
    ...
    $this->db->group_start();
        $this->db->where('country_id', $country_id);
        $this->db->or_where('country_id', NULL);
  ...
  public function get_product_available_quantity($product_id, $country_id )
  {
    $this->db->group_start();
        $this->db->where('product_id', $product_id);
        $this->db->where('serial_status', 0);
  ...
  public function get_product_country_data($product_id, $country_id)
  {
   ...
   public function update_products_countries($id,$country_id,$products_countries_data)
    ...
    public function update_product_countries($product_id, $country_id, $updated_data)
    {
        $this->db->where('product_id', $product_id);
        $this->db->where('country_id', $country_id);
    ...
    public function get_product_main_optional_fields($product_id, $lang_id)
    {
        $this->db->select('products_optional_fields.*, optional_fields_translation.label, optional_fields_translation.text,
                           optional_fields.*, form_fields_types.*, products_translation.title as product_name,
                           optional_fields.id as id');
        ...
        $this->db->where('products_optional_fields.primary_option_id', 0);
    ...
    */
    ...
    public function get_user_optional_fields($cart_product_id, $lang_id)
    {
        ...
        $this->db->join('optional_fields', 'user_products_optional_fields.product_optional_field_id = optional_fields.id');
        ...
        $this->db->order_by('user_products_optional_fields.id', 'asc'); // Added By Mrzok
   ...
   public function get_products_countries_totals()
    ...
    {
        $this->db->select('gallery_images.*, products_images.image_id');
    ...
    public function get_product_optional_field_cost_sec($product_id, $field_id, $lang_id, $primary_id=0)
    {
        ...
        $this->db->join('optional_fields_options_translation', 'products_optional_fields_options_costs.option_id = optional_fields_options_translation.optional_field_option_id
                      AND optional_fields_options_translation.lang_id='.$lang_id, 'left');
    ...
    public function get_product_optional_field_data($id)
    {
        $this->db->where('id', $id);
        $query = $this->db->get('products_optional_fields');
    ...
    public function update_product_optional_field($id, $data)
    {
        $this->db->where('id', $id);
        return $this->db->update('products_optional_fields', $data);
    }

    public function update_products_optional_fields_costs($id, $data)
    {
        $this->db->where('id', $id);
        return $this->db->update('products_optional_fields_options_costs', $data);
    }

    public function get_product_optional_field_cost_row($id)
    {
        $this->db->where('id', $id);
        $query = $this->db->get('products_optional_fields_options_costs');
    ...
    public function remove_product_optional_field($product_id, $option_id)
    {
        $this->db->where('product_id', $product_id);
        ...
        // delete secondary optional fields
        $this->db->where('product_id', $product_id);
        $this->db->where('primary_option_id', $option_id);
        $this->db->delete('products_optional_fields');
    ...
    public function products_optional_fields_options_costs_row($product_id, $optional_field_id)
    {
        $this->db->where('product_id', $product_id);
        $this->db->where('option_id', $optional_field_id);
    ...
    public function check_product_in_compare($product_id, $user_id)
    {
        $this->db->where('product_id', $product_id);
        $this->db->where('user_id', $user_id);
    ...
    public function check_op_group_price_exist($condtions)
    {
        ...
        $count = $this->db->count_all_results('optional_fields_customer_groups_prices');
    ...
    public function get_op_groups_prices($optional_field_id, $option_id)
    {
        $this->db->where('optional_field_id', $optional_field_id);
        $this->db->where('option_id', $option_id);

        $result = $this->db->get('optional_fields_customer_groups_prices');
    ...
    public function get_category_max_sort($cat_id)
    {
    ...
    public function get_product_rating_data($product_id, $product_all_rates)
    {
      $this->db->select('count(rate) as rating_times, rate, (count(rate)*100/'.$product_all_rates.') as rate_percent');
      $this->db->where('product_id', $product_id);
      $this->db->order_by('rate', 'desc');
      $this->db->group_by('rate');

      $query = $this->db->get('products_rating');
