...
class Admin_purchase_orders extends CI_Controller
{
    ...
    public function add()
    {
        ...
        {
            $price = array();
            $option_fields = $this->input->post('option_fields', true);
            $option_fields_values = $this->input->post('op_fields_values', true);
            ...
            if($this->config->item('business_type') == 'b2b')
            {
              ...
              $order_store_id = $this->config->item('default_store_id');
            }
            ...
            /**********************purchase_orders_products data****************************/
             ...
             $serials_count  = 0;
             $file_path   = $this->upload("userfile");

             if(isset($_FILES) && $file_path != '')
             {
               $upload_data = $this->csvreader->parse_file($file_path, true, ';');
               $rows_count = count($upload_data);
               
               $page   = 1;
               $limit  = 300;
               $pages_count = ceil($rows_count/$limit);
               
               for($page=1;$page<=$pages_count;$page++)
               {
                   $offset = ($page -1) * $limit;
                   $to = $offset + $limit;  // array_limit
                   $to_be_uploaded_data = array_slice($upload_data, $offset, $to);
                   
                   foreach($to_be_uploaded_data as $key=>$val)
                   {
                        
                     $quantity[]        = $val['QTY'];
                     $price_per_unit[]  = $val['cost'];
                     $is_optional_field = isset($val['field'])?$val['field']:0;
                     $code              = $val['code'];
                     $price[]           = $val['price'];
                     
                     
                     if($is_optional_field == 0)
                     {
                       $product_data = $this->products_model->get_table_data('products', array('code'=>$code), 'row');
                       ...
                       if(count($product_data) != 0)
                       {
                         ...
                         $option_fields[] = 0;
                         $option_fields_values[] = 0;
                         
                       }
                     }
                     else
                     {
                       $product_data = $this->products_model->get_table_data('products_optional_fields_options_costs', array('code'=>$code), 'row');
                       if(count($product_data) != 0)
                       {
                         $product_ids[]           = $product_data->product_id;
                         $option_fields[]         = $product_data->optional_field_id;
                         $option_fields_values[]  = $product_data->option_id;
                                              
                       }
                     } //END OPTIONAL FIELDS CONDITION
             ...
             foreach($product_ids as $key => $product_id)
             {
                ...
                $purchase_orders_products_data = array(
                                                        ...
                                                        'optional_fields'     => $option_fields[$key]   ,
                                                        'selected_optional_fields' => $option_fields_values[$key]
                                                     );
                ...
                */
                ...
                $pr_data = array('average_cost' => $price_per_unit[$key]);
                
                if($price[$key] != 0)
                {
                    $pr_data['price'] = $price[$key];
                }
//echo '<br />---------------------------------------<br />key: '.$key.'--product id:'.$product_id.'--price: '.$price[$key].'<br>';
...
//echo '<br />product id: '.$product_id.'<pre>';print_r($pr_data);
                $this->purchase_orders_model->update_products_country_average_cost($product_id, $country, $pr_data);
                ...
                ***/
                ...
                /*foreach($option_fields as $index=>$option_id)
                {
                    $purchase_products_ops = array(
                                                    'option_id'  => $option_id,
                                                    'selected_option_id'  => $option_fields_values[$index],
                                                    'purchase_order_id'   => $purchase_order_id     ,
                                                    'product_id'          => $product_id            ,
                                                    'price_per_unit'      => $price_per_unit[$key]  ,
                                                    'country_id'          => $product_country       ,
                   ...
                   $this->purchase_orders_model->insert_table_data('purchase_orders_products_optional_fields', $purchase_products_ops);
                }
                */
    ...
    public function upload($file)
    {
       /******************************Upload file data*****************************/
        ...
        $config['upload_path']   = $gallery_path;
  	    $config['allowed_types'] = 'csv';//'doc|docx|txt|csv|odt|xls|xlsx|pdf|ppt|pptx|pps|ppsx';
    ...
    public function get_products()
    {
        ...
        if(isset($products) && count($products)!= 0)
        {
            ...
            {
                 $options .= "<option value='$row->id'>$row->title - $row->code.</option>";
            }
    ...
    public function get_product_details($product_id)
    {
        ...
        $product_data = $this->products_model->get_products_row($product_id);
        $product_optional_fields = $this->products_model->get_product_optional_fields($product_id, $this->lang_row->id, 0, 'all');
        $op_options   = '';
        $options      = '';

        if(count($product_optional_fields) != 0)
        {
          //optional_fields_exist
          /*foreach($product_optional_fields as $op_field)
          {
            $op_options .= "<option value='$op_field->id'>$op_field->label</option>";
          }*/

          foreach($product_optional_fields as $op_field)
          {
            $option_options = $this->optional_fields_model->get_option_options_data($op_field->id , $this->data['lang_id']);

            $op_options .= '<div class="form-group">
                              <label class="control-label col-md-3">'.$op_field->label.'</label>
                              <div class="col-md-4">
                                <select name="op_'.$op_field->id.'" style="width:100%" class="form-control select2 op_fields_value">';
            foreach ($option_options as $row)
            {
                $op_options .= '<option value="'.$row->id.'">'.$row->field_value.'</option>';
            }
            ...
            <input type="hidden" name="op_fields[]" value="'.$op_field->id.'" class="op_fields" />';
          }
        }
        ...
        echo json_encode(array($options, $op_options));
    }
