...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			//if ($this->bcrypt->verify(SHA1($salt . SHA1($salt . SHA1($password))), $hash_password_db->password))
            if ($this->bcrypt->verify($password, $hash_password_db->salt, $hash_password_db->password))
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query = $this->db->get_where($this->tables['groups_translation'],array('group_id' => $this->config->item('default_group', 'ion_auth')),1)->row();
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			if(!empty($group_names))
			{
                ...
                $this->db->join($this->tables['groups'],$this->tables['groups_translation'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.id', 'inner'. '.' . $this->join['groups'] . ' = ' . $this->tables['groups_translation'] . '.group_id', 'inner');
				$this->db->where_in($this->tables['groups_translation'] . '.name', $group_names);
   	            $this->db->where($this->tables['groups_translation'] . '.lang_id', $lang_id);
			}
			if(!empty($group_ids))
			{
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups_translation'].'.name, '.$this->tables['groups_translation'].'.description')
		                ->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
                        ->where($this->tables['groups_translation'].'.lang_id', 1)
		                ->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id')
                        ->join($this->tables['groups_translation'], $this->tables['groups_translation'].'.'.$this->join['groups'].'='.$this->tables['users_groups'].'.group_id')
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			foreach($group_ids as $group_id)
			{
                ...
                $this->db->delete($this->tables['groups_translation'], array($this->join['groups'] => (int)$group_id, $this->join['groups_translation'] => (int)$group_id));
	...
	public function groups()
	{
        ...
        $this->response   = $this->db->select('groups.*,groups_translation.*')->join('groups_translation','groups.id = groups_translation.group_id')->get('groups');
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
        ...
        $existing_group = $this->db->get_where($this->tables['groups_translation'], array('name' => $group_name))->num_rows();
		if($existing_group !== 0)
		{
		...
		// report success
        $this->db->insert($this->tables['groups_translation'], $data);
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		//	$existing_group = $this->db->get_where($this->tables['groups'], array('name' => $group_name))->row();
            $existing_group = $this->db->get_where($this->tables['groups_translation'], array('name' => $group_name))->row();
		...
		$this->db->update($this->tables['groups_translation'], $data, array('group_id' => $group_id));
	...
	**/
	public function delete_group($group_id = FALSE)
	{
        ...
        $this->db->delete($this->tables['groups_translation'], array($this->join['groups'] => $group_id));
