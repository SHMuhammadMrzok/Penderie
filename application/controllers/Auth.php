...
class Auth extends CI_Controller {
	...
	function index()
	{
		...
		{
            $_SESSION['message'] = lang('activation_email_successful');
	...
	function login()
	{
		...
		else
		{
            ...
            //$this->_render_page(base_url().'users/users/user_login', $this->data);
		}
	}
	...
	function change_password()
	{
		...
		if (!$this->ion_auth->logged_in())
		{
		    $_SESSION['message'] = lang('already_logged_in');
            $this->session->mark_as_flash('message');
	...
	{
	    redirect('users/users/forget', 'refresh');
	...
	function activate($id, $code=false)
	{
		...
		if ($activation)
		{
            ...
            $this->notifications->create_notification('welcome_user', $data, $emails, $phone);
            /**** ---------------------------------------- ****/

			//redirect them to the auth page
			$_SESSION['message'] = lang('welcome_new_active_user');
            $this->session->mark_as_flash('message');

            //redirect(base_url(), 'refresh');
            $this->activate_user_phone($id);
		}
		else
		{
    ...
    public function activate_user_phone($user_id)
    {
        $user = $this->user_model->get_row_data($user_id);

        if($user->account_sms_activated == 1)
        {
        ...
        if($this->form_validation->run() == TRUE)
        {
            ...
            if($user)
            {
                //$sms_code = $this->input->post('sms_code');
								...
								$sms_code = $code1.$code2.$code3.$code4;
                ...
                else
                {
                     ...
                     redirect(base_url().'auth/activate_user_phone/'.$user->id, 'refresh');
                }
            }
        }
        else
        {
            ...
            $this->data['content']  = $this->load->view('users/sms_activate', $this->data, true);
